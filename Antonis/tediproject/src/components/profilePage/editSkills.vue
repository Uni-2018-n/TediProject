<template>
    <div class="external">
        <div class="internal">
            <input type="text" v-model="temp" autocomplete="off" id="text" placeholder="Append new skill" @keyup.enter="append"/>
            <span class="msg">Click on a skill to remove</span>
            <ul class="skills">
                <li v-for="(skill, index) in tempAppend" :key="index">
                    <span @click="remove(index)" class="span-skill">{{ skill }}</span>
                </li>
            </ul>
